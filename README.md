...
Работает в этом Laravel-приложении на хостинге. Пользовательский бот и его polling bridge
не меняются. Новый бот получает команды напрямую через защищённый Telegram webhook.
...
2. Заполните `.env` на хостинге (не коммитьте токены):
   ...
   MONITOR_WEBHOOK_SECRET=случайная_строка_из_32_или_более_символов
   MONITOR_ADMIN_IDS=ваш_числовой_Telegram_ID
   MONITOR_NODE_IDS=parser-vm-1,parser-vm-2
   ...
   `MONITOR_NODE_IDS` должен точно совпадать с уникальными `LOCKED_BY_PREFIX` ваших VM.
   При пустом списке используются все VM, когда-либо приславшие heartbeat; явный список
   предпочтителен: обнаруживает даже VM, ни разу не вышедшую на связь, и позволяет убрать
   выведенную из эксплуатации VM без удаления записей из БД.

   Для `MONITOR_WEBHOOK_SECRET` сгенерируйте 64 hex-символа:

   ```bash
   /opt/php/8.4/bin/php -r 'echo bin2hex(random_bytes(32)), PHP_EOL;'
   ```

3. Примените настройки и зарегистрируйте webhook **нового** бота:

   ```bash
   /opt/php/8.4/bin/php artisan config:cache
   /opt/php/8.4/bin/php artisan monitor:webhook
   ```

   Команда меняет webhook и список команд только бота с `MONITOR_BOT_TOKEN`.
   Не используйте токен пользовательского бота. Администраторы должны открыть нового бота
   и нажать Start (`/start`), иначе Telegram не позволит ему отправлять уведомления.
   Endpoint: `/api/telegram/monitor/webhook`; он проверяет секретный заголовок
   [Telegram Bot API](https://core.telegram.org/bots/api#setwebhook). Пустой секрет закрывает доступ.
...
бот тоже недоступен. При недоступности БД автоматические проверки также не работают;
