...
## Единые права кабинета и бота

Telegram проверяет `workplan.view` для расписания, деталей рейсов и старых inline-кнопок,
а `history.view` — для ознакомления и новых уведомлений об изменениях. Отзыв прав не удаляет
ранее доставленные сообщения. Пункты расписания в меню показываются только при наличии права.
Администратор в обоих интерфейсах определяется ролью `administrator`, не `telegram_accounts.is_admin`.
Перед выкладкой проверьте роли прежних Telegram-администраторов: один старый флаг больше не даёт прав.
При необходимости `php artisan telegram:make-admin TELEGRAM_ID` назначает общую роль администратора
(включая права в кабинете). Автоматического переноса прав со старого флага нет; миграции не нужны.
При переходе `pending → active` в кабинете Telegram-уведомление отправляется после фиксации транзакции.
Повторное сохранение активного пользователя не отправляет его повторно. Ошибка Telegram не отменяет
одобрение и записывается в лог; отдельная очередь повторной доставки не добавлена.
...
Ручные записи (`source=manual`), включая бесплатный премиум, видны только в истории администратора.
Пользовательская история исключает их на уровне API; срок и статус подписки по-прежнему учитывают эти записи.
...
Исключение: подробности изменений со статусом `pending` доступны без подписки для ознакомления.

Пробного срока нет. Старым пользователям автоматически ничего не начисляется.
Закрытые отчёты/фильтры возвращают HTTP 402, календарь — 403, история (кроме `pending`) возвращается с пустым `changes`,
