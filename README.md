...
2. Заполните `.env` на хостинге (не коммитьте токены):
   ...
   MONITOR_BRIDGE_NAME=oscalendar_monitor
   MONITOR_BRIDGE_SECRET=случайная_строка_из_32_или_более_символов
   ...
   Для `MONITOR_BRIDGE_SECRET` можно использовать уже созданный `MONITOR_WEBHOOK_SECRET`:
   просто перенесите значение в новую переменную. Если `MONITOR_BRIDGE_SECRET` не задана,
   код временно использует старую переменную; заданная, но пустая новая переменная закрывает
   доступ. Секрет обязателен, заголовков с именем бота недостаточно для проверки отправителя.
   Если секрета ещё нет, сгенерируйте 64 hex-символа:

   ```bash
   /opt/php/8.4/bin/php -r 'echo bin2hex(random_bytes(32)), PHP_EOL;'
   ```
...
3. Примените настройки и подготовьте **нового** бота к работе через мост:
   ...
   X-TG-Bot: oscalendar_monitor
   X-TG-Bridge-Secret: значение_MONITOR_BRIDGE_SECRET
   ```

   ВАЖНО: схема выше подтверждена существующим примером конфигурации моста, но его исходник
   не входит в этот репозиторий. Способ задания `X-TG-Bridge-Secret` нужно проверить в вашей
   версии моста; поля конфигурации для него здесь намеренно не придуманы. Если мост ещё
   не умеет передавать этот заголовок, его нужно добавить в код отправки запроса на `target`.
   При ответе бэкенда не-2xx мост должен повторять доставку, не продвигая offset для этого
   сообщения. Не помещайте токен или секрет в `target` URL и не отключайте проверку секрета.
   ...
   это **приёмник сообщений моста**, а не прямой webhook Telegram. Запросы без всех трёх
   верных заголовков отклоняются; команды чужих пользователей и из групп игнорируются.
