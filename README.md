...
Администратор в личном кабинете открывает **Пользователи → Оплаты** и вносит подтверждённую
оплату: сумму в рублях, дату оплаты и срок 30/90/180/365 дней. При необходимости можно указать
дату начала старой оплаты и комментарий. Без даты начало — сейчас; действующий срок
продлевается с конца последнего неотменённого оплаченного периода. Дни считаются в UTC.
Повтор запроса с тем же `request_id` не начисляет срок дважды.

Ошибочную запись можно отменить с причиной. История, автор и причина сохраняются;
